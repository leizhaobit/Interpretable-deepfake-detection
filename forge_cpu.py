...
def eval(net, dataloader):
    ...
    for step, (inputs, labels) in enumerate(dataloader, 0):
        ...
        if step % 20 == 0 and step != 0:
            ...
            break
    for clsf in correct_summary.keys():
        correct += correct_summary[clsf] / ((step + 1) * int(inputs.shape[0]))
    acc = correct / 3
    print(acc)
    return acc
...
def run(pretrained_model):
    ...
    for epoch in range(50):
        ...
        if temp_accuracy > accuracy:
            ...
            #torch.save(net.state_dict(), f'build/racnn_ff.pt')
...
if __name__ == "__main__":
    ...
    build_gif(pattern='@2x', gif_name='racnn_ff++c23')
    build_gif(pattern='@4x', gif_name='racnn_ff++c23')
